...
package me.ayunami2000.psbot.AudioParser.Analysis;
...
public class Klapuri{
	...
	double dee = 0.89;
	public Klapuri(double[] data, double max,PitchDetection mainProgram){
	...
	Vector<Double> detectF0s(double[] whitened, PitchDetection mainProgram){
		...
		while (S.lastElement() >= smax){
			...
			for (int j = 0; j<mainProgram.f0index[index].size();++j){
					...
					for (int i = -1;i <= 1;++i){
						int ind=mainProgram.f0index[index].get(j)+i;
						if (ind < resultsk.length && ind > 0 && tempCancelled[ind] == 0){
						   	//System.out.println(mainProgram.f0index[index].get(j)+" "+mainProgram.freq[mainProgram.f0index[index].get(j)]);
							resultsk[ind]= resultsk[ind]
							+(mainProgram.samplingRate*mainProgram.freq[ind]+alpha)
							/
							(
							((double)mainProgram.f0indHarm[index].get(j))
							*mainProgram.samplingRate*mainProgram.freq[ind]+beta
							)
							*whitened[ind];
							if (whitened[ind]-resultsk[ind] > 0){
								whitened[ind]=
								whitened[ind]
								-resultsk[ind]*dee;
							}else{
	...
	double[] whiten(double[] dataIn,PitchDetection mainProgram){
